export function PipelineVisual({ active, p }: { active: number; p: number }) {
  ...
  return (
    ...
    >
      <path d="M58 274C104 131 231 98 304 213" fill="none" stroke="var(--border-strong)" strokeWidth="1" />
      <path d="M42 299C129 256 205 281 319 239" fill="none" stroke="var(--border)" strokeWidth="1" />
      {nodes.map((_, i) => {
        const x = 58 + i * 35;
        const y = 274 - Math.sin((i / 7) * Math.PI) * 162;
        return (
          <g key={i} style={{ opacity: i <= active ? 1 : 0.22, transition: "opacity 500ms ease" }}>
            <circle cx={x} cy={y} r={i === active ? 6 : 3} fill={i === active ? "var(--signal)" : "currentColor"} />
            <text x={x} y={y + 18} textAnchor="middle" className="font-mono" fontSize="6.5" fill="currentColor" opacity="0.65">
