    ...
    >
      <div className="absolute inset-0 overflow-hidden rounded-sm border border-border bg-surface/70 p-6">
        <div className="flex h-full flex-col justify-between pl-[27%] sm:pl-[31%]">
          <div>
            <p className="font-mono text-[0.62rem] tracking-[0.14em] text-signal uppercase">{feature.category}</p>
            <p className="mt-5 text-sm leading-relaxed text-muted-foreground">{feature.description}</p>
          </div>
          <Button asChild variant="ghost" className="h-auto w-fit px-0 py-2 text-xs text-foreground hover:bg-transparent">
            <a href={feature.href}>
      ...
      <button
        ...
        className={`absolute inset-0 z-10 flex origin-left flex-col justify-between overflow-hidden rounded-sm border border-border-strong bg-card p-6 text-left shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring ${
          reduced ? "transition-opacity duration-150" : "transition-transform duration-500 ease-out [transform-style:preserve-3d]"
        } ${open ? (reduced ? "pointer-events-none opacity-0" : "[transform:rotateY(-80deg)]") : "opacity-100 [transform:rotateY(0deg)]"}`}
      >
        <div className="flex items-center justify-between gap-4">
          <span className="font-mono text-[0.62rem] tracking-[0.14em] text-muted-foreground">{number}</span>
        ...
        <div>
          ...
          <p className="mt-3 font-mono text-[0.6rem] tracking-[0.12em] text-muted-foreground uppercase">Open to learn more</p>
        </div>
      </button>
    </article>
  );
